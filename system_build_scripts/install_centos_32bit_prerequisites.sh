...
cd modules-3.2.9
./configure --prefix=/opt/sw/32bit/centos/modules/3.2.9c
make
make install
echo ". /opt/sw/32bit/centos/modules/3.2.9c/Modules/3.2.9/init/bash" >> /etc/bashrc
cd ..
...
./configure --prefix=/opt/sw/32bit/centos/python/2.7.3 --enable-shared
make
make install
cd ..
...
prepend-path PATH               /opt/sw/32bit/centos/python/2.7.3/bin

setenv PYTHONPATH               /opt/sw/32bit/centos/python/2.7.3/lib/python2.7/site-packages
prepend-path LD_LIBRARY_PATH    /opt/sw/32bit/centos/python/2.7.3/lib

prepend-path PKG_CONFIG_PATH    /opt/sw/32bit/centos/python/2.7.3/lib/pkgconfig/ 
EOF


. /opt/sw/32bit/centos/modules/3.2.9c/Modules/3.2.9/init/bash
